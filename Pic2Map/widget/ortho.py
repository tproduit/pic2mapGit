...
class viewOrtho_class(QGLWidget):
    # Emit the bounding box for saving the drapped picture
    getBound = pyqtSignal(list)
    ...
    def __init__(self, pointBuffer, picture_name, modelview, projection, viewport,
        ...
        self.boxLeftUp = [
            self.numpy_verts[indexLD][0], self.numpy_verts[indexLD][2]]
        self.boxRightDown = [
            self.numpy_verts[indexRU][0], self.numpy_verts[indexRU][2]]
    ...
    def mousePressEvent(self, event):
        ...
        if(event.buttons() & Qt.LeftButton):
            ...
            print x, y, z
            result = gluUnProject(
                x, y, z, self.modelview_new, self.projection_new, self.viewport_new)
    ...
    def mouseMoveEvent(self, event):
        if(event.buttons() & Qt.LeftButton):
            ...
            result = gluUnProject(
                x, y, z, self.modelview_new, self.projection_new, self.viewport_new)
            # Dragging the bounding box can only be done from up-left to right
            # down corner
    ...
    def paintGL(self):
        ...
        gluLookAt(self.numpy_verts[index][0], self.maximum + 5000, self.numpy_verts[index][2],
                  self.numpy_verts[index][
                      0], self.maximum, self.numpy_verts[index][2],
                  0.0, 0.0, 1.0)

        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        glOrtho(-self.offsetEst, self.offsetEst, -self.offsetNorth,
                self.offsetNorth, -1000, 10000)
        ...
        if not self.orthoSavedParam:
            ...
            glBegin(GL_QUADS)
            glVertex3f(
                self.boxLeftUp[0], self.maximum + 3000, self.boxLeftUp[1])
            glVertex3f(
                self.boxRightDown[0], self.maximum + 3000, self.boxLeftUp[1])
            glVertex3f(
                self.boxRightDown[0], self.maximum + 3000, self.boxRightDown[1])
            glVertex3f(
                self.boxLeftUp[0], self.maximum + 3000, self.boxRightDown[1])
            glEnd()
            glDepthMask(GL_TRUE)
    ...
    def getViewPortZoom(self):
        winx, winy, winz = gluProject(self.boxLeftUp[0], 0,  self.boxLeftUp[
                                      1], self.modelview_new, self.projection_new, self.viewport_new)
        winx2, winy2, winz2 = gluProject(self.boxRightDown[0], 0,  self.boxRightDown[
                                         1], self.modelview_new, self.projection_new, self.viewport_new)
        self.ParamViewport = [winx / self.viewport_new[2], winy2 / self.viewport_new[
            3], winx2 / self.viewport_new[2], winy / self.viewport_new[3]]
    ...
    def saveOrtho(self, name=None):
        ...
        imgSave = Image.fromstring("RGB", (self.orthoViewPort[
                                   2] - self.orthoViewPort[0], self.orthoViewPort[3] - self.orthoViewPort[1]), data)
        if name == None:
            imageSaveName = QFileDialog.getSaveFileName(
                self, "save file dialog", "/raster.tiff", "Images (*.tiff *.png)")
            if imageSaveName:
                format = imageSaveName.split('.')[1]
                try:
                    if format == 'tiff':
                        # In case the user save the raster as tiff, it get
                        # georeferenced (so geotiff)
                        raster = array(
                            imgSave.transpose(Image.FLIP_TOP_BOTTOM))
                        LeftUpInt = gluUnProject(int(self.orthoViewPort[0]), int(self.orthoViewPort[
                                                 3]), 0.5, self.modelview_new, self.projection_new, self.viewport_new)
                        vectReference = [-LeftUpInt[0], self.meterPerPixel,
                                         0, LeftUpInt[2], 0,  -self.meterPerPixel]
                        self.WriteGeotiff(
                            raster, imageSaveName, gdal.GDT_Byte, self.crs, vectReference)
                    elif format == 'png':
                        # In cas the user save the raster as png, it is not
                        # georeferenced
    ...
    def WriteGeotiff(self, raster, filepath, dtype, crs, vectReference):
        ...
        dst_ds = driver.Create(
            filepath, ncols, nrows, nbands + 1, dtype, ['COMPRESS=LZW'])
    ...
    def initializeGL(self):
        if self.orthoSavedParam:
            if self.isFrameBufferSupported:
                ...
                glRenderbufferStorage(
                    GL_RENDERBUFFER, GL_RGB, self.totPixE, self.totPixN)
                glBindFramebuffer(GL_FRAMEBUFFER, self.fbo)
                glFramebufferRenderbuffer(
                    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, self.render_buf)
        ...
        glBindBufferARB(GL_ARRAY_BUFFER_ARB, self.m_nVBOVertices)
        glBufferDataARB(
            GL_ARRAY_BUFFER_ARB, self.numpy_verts, GL_STATIC_DRAW_ARB)

        glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0)
        glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, self.m_indicebuffer)
        glBufferDataARB(
            GL_ELEMENT_ARRAY_BUFFER_ARB, self.m_indices, GL_STATIC_DRAW_ARB)
        glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB, 0)
        glBindBufferARB(GL_ARRAY_BUFFER_ARB, self.m_texbuffer)
        glBufferDataARB(
            GL_ARRAY_BUFFER_ARB, self.numpy_texture, GL_STATIC_DRAW_ARB)
        glBindBufferARB(GL_ARRAY_BUFFER_ARB, 0)
