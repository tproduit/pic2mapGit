...
class ReportDialog(QtGui.QDialog):
    ...
    def __init__(self, model, Qxx, paramBool, paramList, pathToData, xyzUnProjected):
        ...
        if len(errors3D) > 0:
            ...
            text += '\n   mean: ' + \
                str(round(mean(errorsPixels), 2)) + ' [pixel]'
            text += '\n   min: ' + str(min(errorsPixels)) + ' [pixel]'
            text += '\n   max: ' + str(max(errorsPixels)) + ' [pixel]'
            text += '\n   std: ' + \
                str(round(std(errorsPixels), 2)) + ' [pixel]'
            ...
            paramText = ['X position', 'Y Position', 'Z Position',
                         'Tilt [rad]', 'Heading [rad]', 'Swing [rad]', 'Focal [pixel]']
    ...
    def saveReport(self):
        # Save a small text file which contain the same information as the
        # dialog.
    ...
    def isBehindHill(self, paramList, errors3D):
        ...
        for xyz, error in zip(self.xyzUnProjected, errors3D):
            x = xyz[0]
            y = xyz[2]
            planimetricDistanceToCameraPosition = sqrt(
                (x - x0) ** 2 + (y - y0) ** 2)
