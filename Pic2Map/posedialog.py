...
from numpy import zeros, array, sin, cos, dot, linalg, pi, asarray, mean, shape,\
    sqrt, flipud, std, concatenate, ones, arccos, arcsin,\
    arctan, arctan2, size, abs, matrix, diag
...
class Pose_dialog(QtGui.QDialog):
    ...
    def reportOnGCPs(self):
        self.report = ReportDialog(
            self.model, self.Qxx, self.parameter_bool, self.result, self.pathToData, self.xyzUnProjected)
    ...
    def estimatePose(self):
        ...
        for radioButton in self.findChildren(QtGui.QRadioButton):
            if radioButton.text() == "Fixed":
                if radioButton.isChecked():
                    ...
                    if indice == 7:
                        # central point is displayed in reference to the center
                        # of image
        ...
        try:
            ...
            if DltBool:
                ...
                resultInitialization = [0, 0, 0, pi, 0, 0, sqrt(
                    self.sizePicture[0] ** 2 + self.sizePicture[0] ** 2), 0, 0]
            ...
            """
            resultLS, Lproj, vect = self.LS(
                xyz, uv1, parameter_bool, parameter_list, resultInitialization)

            k = 0
            l = 0
            result = [0] * 9
            # Length of resultLS is [9 - length of parameter_list]
            # We reconstruct the "result" vector which contains the output
            # parameters
            for i in range(9):
                if parameter_bool[i]:
                    result[i] = resultLS[k]
            ...
            self.lookat = vect
            self.pos = result[0:3]
            # The focal, here calculate in pixel, has to be translated in term
            # of vertical field of view for openGL
            self.FOV = (
                2 * arctan(float(self.sizePicture[1] / 2.0) / result[6])) * 180 / pi
    ...
    def LS(self, abscissa, observations, PARAM, x_fix, x_ini):
        # The initial parameters are the ones from DLT but where the radio
        # button is set as free
        x = []
        for i in range(9):
            if PARAM[i]:
                x.append(x_ini[i])
        ...
        # The P matrix is a weight matrix, useless if equal to identity (but
        ...
        # v_test contain the residual errors between observations and
        # predictions after an update of H
        ...
        dx[0] = 1
        # Lambda is the weightage parameter in Levenberg-marquart between the
        # gradient and the gauss-newton parts.
        Lambda = 0.01
        # increment used for Jacobian and for convergence criterium
        inc = 0.001
        while (max(abs(dx)) > inc) & (it < maxit):
            # new iteration, parameters updates are greater than the
            ...
            for i in range(npoints):
                ...
                # The difference between the observation and prediction is used
            ...
            try:
                ...
                # In this case, we don't accept the update and go for another
                # iteration
                S2 = S0
            else:
                for i in range(npoints):
                    # We check that the update has brought some good stuff in the pocket
                    # In other words, we check that the sum of square of less
                    # than before (better least square!)
                    utest, vtest = self.dircal(
                        x_test, abscissa[i, :], x_fix, PARAM)
        ...
        R[0, 1] = sin(azimuth) * cos(swing) - \
            cos(azimuth) * cos(tilt) * sin(swing)
        R[0, 2] = -sin(tilt) * sin(swing)
        R[1, 0] = cos(azimuth) * sin(swing) - \
            sin(azimuth) * cos(tilt) * cos(swing)
        R[1, 1] = -sin(azimuth) * sin(swing) - \
            cos(azimuth) * cos(tilt) * cos(swing)
    ...
    def CoeftoMatrixProjection(self, x):
        ...
        L1_line[3] = ((focal * R[0, 0] - u0 * R[2, 0]) * x0 + (focal * R[0,
                                                                         1] - u0 * R[2, 1]) * y0 + (focal * R[0, 2] - u0 * R[2, 2]) * z0) / D
        L1_line[4] = (v0 * R[2, 0] - focal * R[1, 0]) / D
        L1_line[5] = (v0 * R[2, 1] - focal * R[1, 1]) / D
        L1_line[6] = (v0 * R[2, 2] - focal * R[1, 2]) / D
        L1_line[7] = ((focal * R[1, 0] - v0 * R[2, 0]) * x0 + (focal * R[1,
                                                                         1] - v0 * R[2, 1]) * y0 + (focal * R[1, 2] - v0 * R[2, 2]) * z0) / D
        L1_line[8] = R[2, 0] / D
        L1_line[9] = R[2, 1] / D
        L1_line[10] = R[2, 2] / D
        L1_line[11] = 1
    ...
    def DLTcalibration(self, xyz, uv):
        ...
        # Formulating the problem as a set of homogeneous linear equations,
        # M*p=0:
        A = []
        for i in range(npoints):
    ...
    def Normalization(self, nd, x):
        ...
        if nd == 2:
            Tr = array(
                [[std(x[:, 0]), 0, m[0]], [0, std(x[:, 1]), m[1]], [0, 0, 1]])
        else:
            Tr = array([[std(x[:, 0]), 0, 0, m[0]], [0, std(x[:, 1]), 0, m[1]], [
    ...
    def DLTMain(self, xyz, uv1):
        ...
        du2 = D2 * ((u0 * L1[8] - L1[0]) ** 2 + (u0 *
                                                 L1[9] - L1[1]) ** 2 + (u0 * L1[10] - L1[2]) ** 2)
        dv2 = D2 * ((v0 * L1[8] - L1[4]) ** 2 + (v0 *
                                                 L1[9] - L1[5]) ** 2 + (v0 * L1[10] - L1[6]) ** 2)
        du = sqrt(du2)
        dv = sqrt(dv2)
        focal = (du + dv) / 2

        R_mat = array([[(u0 * L1[8] - L1[0]) / du, (u0 * L1[9] - L1[1]) / du, (u0 * L1[10] - L1[2]) / du],
                       [(v0 * L1[8] - L1[4]) / dv, (v0 * L1[9] - L1[5]) / dv,
                        (v0 * L1[10] - L1[6]) / dv],
                       [L1[8], L1[9], L1[10]]])
        ...
        awesome_vector = array(
            almost_awesome_vector) + array([x0y0z0[0, 0], x0y0z0[1, 0], x0y0z0[2, 0]])
