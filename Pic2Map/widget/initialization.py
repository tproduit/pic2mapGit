...
class Initialization_dialog(QtGui.QDialog):
    ...
    def __init__(self):
        ...
        QtCore.QObject.connect(
            openFile, QtCore.SIGNAL('clicked()'), self.showDialog)
        openFile = self.ui.toolButtonDEM
        QtCore.QObject.connect(
            openFile, QtCore.SIGNAL('clicked()'), self.showDialogDEM)

        QtCore.QObject.connect(
            self.ui.buttonBox, QtCore.SIGNAL('helpRequested()'), self.helpWindow)
        QtCore.QObject.connect(
            self.ui.checkBox, QtCore.SIGNAL('stateChanged(int)'), self.orthoActivate)

        QtCore.QObject.connect(
            self.ui.lineEditDEM, QtCore.SIGNAL("dropped"), self.dropEvent)
        self.openOrtho = self.ui.toolButton_2
        QtCore.QObject.connect(
            self.openOrtho, QtCore.SIGNAL('clicked()'), self.showDialogOrtho)
    ...
    def showDialogOrtho(self):
        fname = QtGui.QFileDialog.getOpenFileName(
            self, 'Open file', self.currentPath)
    ...
    def showDialogDEM(self):
        fname = QtGui.QFileDialog.getOpenFileName(
            self, 'Open file', self.currentPath, "Images (*.tiff *.tif)")
