...
from ..ui.ui_buffering import Ui_progressBar
...
class MonoplotterMainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, iface, pointBuffer, picture_name, ParamPose, dem_box, cLayer, pathToData, crs, demName, isFrameBufferSupported):
        ...
        self.ui.activatePolygon.clicked.connect(
            self.activatePolygonVisualization)
    ...
    def openLabelSettings(self):
        ...
        if result == 1:
            self.labelSetting[
                0] = self.labelSettingWindow.ui.colorButton.palette().color(1)
            self.labelSetting[1] = self.labelSettingWindow.font
            self.labelSetting[
                2] = self.labelSettingWindow.ui.doubleSpinBox.value()
            self.labelSetting[
                3] = self.labelSettingWindow.ui.doubleSpinBox_2.value()
    ...
    def getOrtho(self, forPolygon=False):
        ...
        zBuffer = zBuffer.reshape(
            (self.qgl_window.viewport[3], self.qgl_window.viewport[2]))
        zBuffer = zBuffer.T
        bar = progress_bar()
        ...
        matrix_ini = concatenate((self.qgl_window.numpy_verts, ones(
            (self.qgl_window.l_est * self.qgl_window.l_nord, 1))), 1)
        ...
        for winx, winy, winz in win:
            # check if the projection falls inside the sensor plane (id est
            # falls inside the viewport of the QGLwidget
    ...
    def saveImage(self):
        ...
        imageSaveName = QtGui.QFileDialog.getSaveFileName(
            self, "save file dialog", path, "Images (*.png *.xpm *.jpg)")
        if imageSaveName:
    ...
    def get_box(self):
        ...
        box = [self.dem_box.xMinimum(), self.dem_box.yMinimum(),
               self.dem_box.xMaximum(), self.dem_box.yMaximum()]
        a = box[0]
        b = box[1]
        c = box[2]
        d = box[3]
        ...
        if FOV < 90:
            ...
            feat = QgsFeature()
            feat.setGeometry(
                QgsGeometry.fromPolyline([QgsPoint(x_min, y_min), QgsPoint(x_max, y_max)]))
    ...
    def getSymbology(self):
        ...
        # is used for given the right symbol to the right feature according to
        # atttributes
            ...
            try:
                ...
                if layer.type() == 0 and self.iface.legendInterface().isLayerVisible(layer):
                    ...
                    if layer.geometryType() == 0:
                        ...
                        if renderer.type() == "singleSymbol":
                            ...
                            for ran in renderer.ranges():
                                new_graduation.append(
                                    [ran.lowerValue(), ran.upperValue()])
                                test_oneSymbolLayer -= 1
                    ...
                    # Polygon
                        ...
                        # Get only the viewshed, the first time polygons are
                        # activated
                ...
                QMessageBox.warning(
                    self, "Symbology - Error", "Failed update icons, Unsupported Symbology: %s " % e)
        self.qgl_window.defSymbology(
            layerType, symbolParam, symbolGraduations, symbolCategories, symbolColor, symbolSize)
    ...
    def createMaskClip(self, path):
        ...
        self.getOrtho(True)
        meterPerPixel = 10
        totPixN = (self.qgl_window.l_nord / meterPerPixel) * \
            self.qgl_window.resolution
        totPixE = (self.qgl_window.l_est / meterPerPixel) * \
            self.qgl_window.resolution
        ...
        try:
            ...
            while count < 4 and not done:
                try:
                    ...
                    outShapefile2 = path + "intersectioned"
                    outDatasource2 = driver.CreateDataSource(
                        outShapefile2 + ".shp")
                    outLayer2 = outDatasource2.CreateLayer(
                        "intersectioned", None)
                    ...
                    out_lyr = out_ds.CreateLayer(
                        'polygons', geom_type=ogr.wkbPolygon)
                    ...
                    for feature2 in iter2:
                        ...
                        for vertex in B[0]:
                            ident = self.cLayer.dataProvider().identify(
                                vertex, QgsRaster.IdentifyFormatValue).results()
                            value = ident.get(1)
        ...
        for in_feat in in_lyr:
            ...
            if geom.GetGeometryName() == 'MULTIPOLYGON':
                for i in range(geom.GetGeometryCount()):
                    ...
                    self.addPolygon(
                        geom_part.ExportToWkb(), out_lyr, field_value)
    ...
    def preparePurpleCross(self, x, y):
        ...
        ident = dataProvider.identify(
            QgsPoint(x, y), QgsRaster.IdentifyFormatValue).results()
        value = ident.get(1)
                ...
                if layer.geometryType() != 2:
                    ...
                    if layer.geometryType() == 0:  # Point
                        ...
                        for feature in iter2:
                            ...
                            y = p.y()
                            ident = dataProvider.identify(
                                p, QgsRaster.IdentifyFormatValue).results()
                            value = ident.get(1)
                            pointArrayXYZ.append([x, value, y])
                            if attributeTest == 1:
                                # Graduated symbology
                                attributeValue = feature.attributes()[
                                    AttributeID]
                                Index = self.getIndexSizeColorGraduation(
                                    attributeValue, graduation)
                                AttributeToSizeColorArray.append(Index)
                            if attributeTest == 2:
                                # Categorized symbology
                                attributeValue = feature.attributes()[
                                    AttributeID]
                                Index = self.getIndexSizeColorCategorie(
                                    attributeValue, categorie)
                                AttributeToSizeColorArray.append(Index)
                        self.qgl_window.pointRefresh(
                            pointArrayXYZ, AttributeToSizeColorArray, zBuffer, attributeTest, labels)
                        ...
                        self.qgl_window.line2Refresh(
                            LineArrayXYZ, AttributeToSizeColorArray, zBuffer, attributeTest, labels)
                    ...
                    if self.polygonActivated:
                        ...
                        if attributeTest == 0:
                            ...
                            # Graduated symbology
                            AttributeID = self.getAttrId(
                                layerCliped, self.attPolygon[counterPolygon])
