...
from widget.getGCPMainWindow import GetGCPMainWindow
from widget.initialization import Initialization_dialog
from widget.virtual3DMainWindow import Virtual3DMainWindow
from widget.monoplotterMainWindow import MonoplotterMainWindow
from widget.drapping import drappingMain
from widget.ortho import viewOrtho_class
...
class Pic2Map:
    ...
    def checkRequirments(self):
        ...
        # Until now, no hard or software, apart from QGis 2.0 version, has
        # showed problem with the plugin
        self.isSupported = 1
    ...
    def initGui(self):
        ...
        self.action = QtGui.QAction(
            QtGui.QIcon(":/plugins/Pic2Map/icon.png"), "Pic2Map", self.iface.mainWindow())
        QtCore.QObject.connect(
            self.action, QtCore.SIGNAL("triggered()"), self.run)
    ...
    def run(self):
        ...
        if not self.isSupported:
            QtGui.QMessageBox.critical(
                QtGui.QWidget(), "Version - Error", "system configuration insufficient")
            return
        ...
        self.ini.setWindowModality(QtCore.Qt.ApplicationModal)
        ...
        if result == 1:
            try:
                ...
                QMessageBox.warning(
                    QtGui.QWidget(), "I/O - Error", "Failed to initialize picture or DEM")
                return
            if self.DEM_name.split('.')[1] != 'tiff' and self.DEM_name.split('.')[1] != 'tif':
                QtGui.QMessageBox.warning(QtGui.QWidget(
                ), "I/O - Error", """Unable to load DEM. \nDEM must be in geotiff format.
            ...
            if self.useOrtho:
                ...
                try:
                    ...
                    fileInfo = QTCore.QFileInfo(self.ortho_name)
                    baseName = fileInfo.baseName()
                    rlayer = qgisCore.QgsRasterLayer(self.ortho_name, baseName)
                    self.ortho_box = [int(-rlayer.extent().xMinimum()), int(rlayer.extent().yMinimum(
                    )), int(-rlayer.extent().xMaximum()), int(rlayer.extent().yMaximum())]
                except IOError:
                    QtGui.QMessageBox.warning(
                        QtGui.QWidget(), "I/O - Error", "Failed to load ortho-image")
                    return
            ...
            if result == 1:
                ...
                if self.ini.ui.radioButton_2.isChecked():
                    self.virtual3DMainWindow = Virtual3DMainWindow(
                        self.buffers, self.picture_name, self.crs, self.pathToData)
                    self.virtual3DMainWindow.show()
                    self.virtual3DMainWindow.ui.goToMonoplotterButton.clicked.connect(
                        self.goToMonoplotter)
                    self.virtual3DMainWindow.ui.GoToGCP.clicked.connect(
                        self.runGCPMainWindowFromVirtual3D)
    ...
    def goToMonoplotter(self):
        ...
        # and launch the monoplotter. It is not possible anymore to change the
        ...
        # Check if the GCP approach has been used
            ...
            self.ParamPose = [self.gcpMainWindow.pos, self.gcpMainWindow.lookat,
                              self.gcpMainWindow.FOV, self.gcpMainWindow.roll]
        else:
            raise IOError
        ...
        self.monoplotter.ui.measureButton.clicked.connect(
            self.activateMeasurement)
    ...
    def drawPinkCross(self, pos):
        ...
        if pos[2] == 1:
            self.pinkCross = qgisGui.QgsVertexMarker(self.canvas)
            posi = qgisCore.QgsPoint(pos[0], pos[1])
    ...
    def runGCPMainWindow(self, fromVirtual3D):
        ...
        self.clickTool = qgisGui.QgsMapToolEmitPoint(self.canvas)
        self.canvas.setMapTool(self.clickTool)
        # create main window
        self.gcpMainWindow = GetGCPMainWindow(
            self.iface, self.buffers, self.picture_name, self.pathToData, self.isFrameBufferSupported, self.crs)
        ...
        self.gcpMainWindow.ui.tableView.selectionModel().currentRowChanged.connect(
            self.resetClickTool)
        ...
        self.gcpMainWindow.GoToMonoplotterButton.triggered.connect(
            self.goToMonoplotter)
    ...
    def load_dem(self):
        try:
            ...
            baseName = fileInfo.baseName()
            rlayer2 = qgisCore.QgsRasterLayer(fileName, baseName)
            ...
            if not rlayer2.isValid():
                QtGui.QMessageBox.warning(QtGui.QWidget(), "IO - Error",
            ...
            QtGui.QMessageBox.warning(QtGui.QWidget(), "CRS - Error",
    ...
    def load_picture(self, picName):
        ...
        fileInfo = QtCore.QFileInfo(fileName)
        baseName = fileInfo.baseName()
        rlayer = qgisCore.QgsRasterLayer(fileName, baseName)
        resolution = QtGui.QDesktopWidget().screenGeometry()
        size = [0, 0]
        size[1] = resolution.height() / 2
        size[0] = int(
            float(float(rlayer.width()) / float(rlayer.height())) * size[1])
        ...
        # Set the zoom such the picture is almost in full view in the height
        # directon
        factor = float(
            self.gcpMainWindow.ui.graphicsView.size().height()) / rlayer.height()
    ...
    def resetClickTool(self):
        ...
        # It resets the click tool for getting altitude on the DEm layer
        # (self.clayer)
        self.canvas.setMapTool(self.clickTool)
        QtCore.QObject.connect(self.clickTool, QtCore.SIGNAL(
            "canvasClicked(const QgsPoint &, Qt::MouseButton)"), self.newCanvasGCP)
    ...
    def load_buffer(self):
        ...
        # Arrays are created here for openGL. The normal calculation can take
        # some time (ca. 1sec for 100'000 pixels)
        self.buffers = Buffers(
            self.DEM_name, self.dem_box, self.useOrtho, self.ortho_name, self.ortho_box)
        self.buffers.getBuffer()
        self.buffers.close()

    def clickOnMonoplotter(self, customEvent):
        # This function is called when digitalization or measurement on
        # monoplotter
        x = customEvent[0]
        y = customEvent[1]
            ...
            if currentLayer.type() == 0 and currentLayer.isEditable() and currentLayer.geometryType() < 3:
                if self.cTool.isEditTool():
                    if self.iface.actionAddFeature().isEnabled():
                        ...
                        p.setY(v)
                        event = QMouseEvent(
                            QEvent.MouseButtonRelease, p, button, button, Qt.NoModifier)
                ...
                if button == Qt.LeftButton:
                    ...
                    if u_previous != self.u or v_previous != self.v:
                        ...
                        event = QtGui.QMouseEvent(
                            QtCore.QEvent.MouseButtonRelease, p, button, button, QtCore.Qt.NoModifier)
                        self.cTool.canvasReleaseEvent(event)
                        self.monoplotter.qgl_window.lineEditBufferAppend()
                        self.monoplotter.qgl_window.updateGL()
                        self.cTool.activate()
    ...
    def activateMeasurement(self):
        # This function is used for calling the measuring tool from the
        # monoplotter
    ...
    def activeFlackOnMonoplotter(self):
        self.clickTool2 = qgisGui.QgsMapToolEmitPoint(self.canvas)
        ...
        self.monoplotter.closingMonoplot.connect(
            self.disactiveFlackOnMonoplotter)
